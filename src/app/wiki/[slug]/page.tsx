...
export default async function RemoteMdxPage({ params }: { params: { slug: string } }) {
    ...
    return (
        <>
            <nav className='transition-all ease-in-out overflow-hidden flex flex-col border-r border-slate-800 text-light-200'>
                <Link href={'/wiki'} className='p-2 rounded bg-slate-600 bg-opacity-0 hover:bg-opacity-25 transition-all ease-in-out'> welcome </Link>
                {allPosts.map((post) =>
                    <Link key={post.slug} className='p-2 rounded bg-slate-600 bg-opacity-0 hover:bg-opacity-25 transition-all ease-in-out' href={`/wiki/${post.slug}`}>
                        {post.data.title}
                    </Link>
                )}
            </nav>
